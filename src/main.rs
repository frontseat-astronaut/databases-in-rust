...
fn prepare_dbs(include_log_db: bool, include_all_variants: bool) -> VecDeque<Box<dyn KVDb>> {
    ...
    if include_log_db {
        dbs.push_back(Box::new(LogDb::new("db_files/log_db/", "log.txt"))); // too slow
    }
    ...
    if include_all_variants {
        ...
        for merging_threshold in (1..10).step_by(4) {
            for sparsity in (100..=1000).step_by(400) {
                for memtable_size_threshold in (500..=10000).step_by(4000) {
                    dbs.push_back(Box::new(
                        SSTable::new(
                            &format!(
        ...
        dbs.push_back(Box::new(
            SegmentedLogsWithIndicesDb::new("db_files/segmented_logs_with_indices_db/", 10000, 5)
                .unwrap(),
        ));
        dbs.push_back(Box::new(
...
fn main() {
    /* CORRECTNESS TESTS */
    let correctness_test_suite = CorrectnessTest::new(6000, 20000, 0.5, 0.7);
    let dbs = prepare_dbs(false, false);
    run_test_suite(correctness_test_suite, dbs);

    /* LATENCY TESTS */
    let latency_test_suite = LatencyTest::new(50000, 20000, 0.5, 0.7);
    ...
    let correctness_test_suite = CorrectnessTest::new(2000, 10000, 0.5, 0.7);
    let dbs = prepare_dbs(true, false);
    run_test_suite(correctness_test_suite, dbs);

    let latency_test_suite = LatencyTest::new(2000, 10000, 0.5, 0.7);
    let dbs = prepare_dbs(true, false);
    run_test_suite(latency_test_suite, dbs);
}
