...
pub struct SSTable {
    ...
    segmented_db_lock: Arc<RwLock<SegmentedDb<File, Factory>>>,
...
impl KVDb for SSTable {
    ...
    fn get(&self, key: &str) -> Result<Option<String>, Error> {
        ...
        self.segmented_db_lock.read()?.get(key)
    }
}
...
impl SSTable {
    pub fn new(
        ...
        Ok(SSTable {
            ...
            segmented_db_lock: Arc::new(RwLock::new(SegmentedDb::<File, Factory>::new(
                dir_path,
                merging_threshold,
    ...
    fn flush_tmp_memtable_in_background(&mut self) {
        ...
        let segmented_db_lock = Arc::clone(&self.segmented_db_lock);
        self.join_handle = Some(spawn(move || {
            if let Err(e) = Self::flush_tmp_memtable(tmp_memtable_lock, segmented_db_lock) {
    ...
    fn flush_tmp_memtable(
    ...
    ) -> Result<(), Error> {
        {
            ...
            let mut segmented_db = segmented_db_lock.write()?;
            segmented_db
            ...
            for (key, entry) in tmp_memtable.iter() {
                match entry {
                    Present(value) => segmented_db.set(key, value)?,
                    Deleted => segmented_db.delete(key)?,
                }
            }
        }
